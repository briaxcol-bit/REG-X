...
import { DollarSign, Loader2, Unlock, Play } from 'lucide-react'
...
interface OpenCashModalProps {
  open: boolean
  isCommandsOnly?: boolean   // true → modo comanda, sin efectivo
  terminalName?: string      // nombre sugerido del terminal
}
...
export function OpenCashModal({ open, isCommandsOnly = false, terminalName }: OpenCashModalProps) {
  const { branch } = useAuthStore()
  const currency = branch?.currency ?? 'COP'
  const { open: openMutation } = useCashSession()

  const defaultName = isCommandsOnly
    ? (terminalName ?? 'Turno Comanda')
    : 'Caja Principal'
  ...
  const openingCash = isCommandsOnly
    ? 0
    : useDenomin
      ? totalFromDenom
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    openMutation.mutate({ name: name.trim() || defaultName, openingCash })
  }
          ...
          >
            ...
            <div className="flex items-center gap-3 border-b border-grafito-100 dark:border-white/5 px-6 py-5">
              <div className={cn(
                ...
                isCommandsOnly ? 'bg-amber-500/10' : 'bg-brand-500/10'
              )}>
                {isCommandsOnly
              ...
              <div>
                <h2 className="font-bold text-grafito-900 dark:text-white">
                  {isCommandsOnly ? 'Iniciar Turno' : 'Apertura de Caja'}
                </h2>
                <p className="text-xs text-grafito-500">
                  {isCommandsOnly
                    ? 'Inicia tu turno para comenzar a tomar pedidos'
                    : 'Registra el efectivo inicial para comenzar'
                  }
                </p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div>
                <label className="text-xs font-semibold text-grafito-700 dark:text-grafito-300 block mb-1.5">
                  {isCommandsOnly ? 'Nombre del turno' : 'Nombre de la caja'}
                </label>
                <input
                  value={name}
              ...
              {/* Campos de efectivo — solo si NO es comanda */}
              {!isCommandsOnly && (
                <>
              ...
              {isCommandsOnly && (
                <div className="rounded-xl bg-amber-500/10 border border-amber-500/20 px-4 py-3 flex items-start gap-3">
                  ...
                  <p className="text-xs text-amber-700 dark:text-amber-400 leading-relaxed">
                    Este punto de caja opera en modo <strong>comanda</strong>. No requiere efectivo inicial — solo toma pedidos e imprime tickets para los clientes.
                  </p>
                </div>
              )}
              ...
              <button
                ...
                className={cn(
                  'w-full flex items-center justify-center gap-2 rounded-xl py-3 text-sm font-bold text-white disabled:opacity-60 transition-colors',
                  isCommandsOnly
              ...
              >
                {openMutation.isPending ? (
                  <><Loader2 className="h-4 w-4 animate-spin" /> {isCommandsOnly ? 'Iniciando…' : 'Abriendo caja…'}</>
                ) : isCommandsOnly ? (
