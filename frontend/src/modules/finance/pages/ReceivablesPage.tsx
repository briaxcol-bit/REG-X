...
import { CircleDollarSign, Plus, Loader2, X } from 'lucide-react'
import { cn } from '@shared/utils/cn'
import { useAuthStore } from '@store/auth.store'
import { formatCurrency } from '@shared/utils/format'
import { getReceivables, createReceivable, addReceivablePayment, getCustomers, type ReceivableRow } from '@lib/db'
...
function CreateModal({ tenantId, onClose }: { tenantId: string; onClose: () => void }) {
  ...
  const save = useMutation({
    mutationFn: () => createReceivable(tenantId, { customer_id: customerId || null, description: desc.trim() || null, reference: reference.trim() || null, amount: Number(amount) || 0, due_date: due || null }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['receivables', tenantId] }); toast.success('Cuenta creada'); onClose() },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(3,7,18,0.75)', backdropFilter: 'blur(6px)' }} onClick={onClose}>
      <div className="w-full max-w-md rounded-2xl bg-white dark:bg-grafito-900 border border-grafito-200 dark:border-white/10 p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-base font-bold text-grafito-900 dark:text-white">Nueva cuenta por cobrar</h3>
...
function PayModal({ tenantId, row, onClose }: { tenantId: string; row: ReceivableRow; onClose: () => void }) {
  const qc = useQueryClient()
  const [amount, setAmount] = useState('')
  const [method, setMethod] = useState('CASH')
  const balance = Number(row.amount) - Number(row.paid)
  const pay = useMutation({
    mutationFn: () => addReceivablePayment(tenantId, row.id, { amount: Number(amount) || 0, method }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['receivables', tenantId] }); toast.success('Abono registrado'); onClose() },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(3,7,18,0.75)', backdropFilter: 'blur(6px)' }} onClick={onClose}>
      <div className="w-full max-w-sm rounded-2xl bg-white dark:bg-grafito-900 border border-grafito-200 dark:border-white/10 p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-base font-bold text-grafito-900 dark:text-white">Registrar cobro</h3>
          <button onClick={onClose} className="rounded-lg p-1.5 text-grafito-400 hover:bg-grafito-100 dark:hover:bg-white/5"><X className="h-4 w-4" /></button>
        </div>
        ...
        <div className="flex gap-2 mt-5">
          ...
          <button onClick={() => pay.mutate()} disabled={!amount || Number(amount) <= 0 || pay.isPending} className="flex-1 inline-flex items-center justify-center gap-2 rounded-xl bg-brand-500 py-2.5 text-sm font-semibold text-white hover:bg-brand-600 disabled:opacity-50">
            {pay.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <CircleDollarSign className="h-4 w-4" />} Cobrar
          </button>
        </div>
      </div>
    </div>
...
export default function ReceivablesPage() {
  ...
  const [pay, setPay] = useState<ReceivableRow | null>(null)

  const { data: rows = [], isLoading } = useQuery({ queryKey: ['receivables', tenantId], queryFn: () => getReceivables(tenantId!), enabled: !!tenantId })
    ...
    <div className="space-y-6 p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="rounded-xl bg-brand-500/10 p-2.5"><CircleDollarSign className="h-5 w-5 text-brand-500" /></div>
          <div>
            <h1 className="text-2xl font-bold text-grafito-900 dark:text-white tracking-tight">Cuentas por Cobrar</h1>
            <p className="text-sm text-grafito-500 dark:text-grafito-400">Cartera de clientes y cobros.</p>
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-grafito-200 dark:border-white/5 bg-white dark:bg-grafito-900/60 p-4 max-w-xs"><p className="text-xs text-grafito-500">Total por cobrar</p><p className="text-2xl font-black text-brand-500 mt-1">{formatCurrency(outstanding, 'COP')}</p></div>
      ...
      <div className="rounded-2xl border border-grafito-200 dark:border-white/5 bg-white dark:bg-grafito-900/60 overflow-x-auto">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center gap-2 py-16 text-grafito-400"><CircleDollarSign className="h-8 w-8" /><p className="text-sm">Sin cuentas por cobrar.</p></div>
        ) : (
          <table className="w-full text-left text-sm min-w-[820px]">
            <thead><tr className="text-[11px] font-semibold uppercase text-grafito-500 border-b border-grafito-200 dark:border-white/5">
              <th className="px-4 py-3">Cliente</th><th className="px-4 py-3">Descripción</th><th className="px-4 py-3 text-right">Monto</th><th className="px-4 py-3 text-right">Saldo</th><th className="px-4 py-3">Estado</th><th className="px-4 py-3 text-right">Acción</th>
            </tr></thead>
              ...
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.id} className="hover:bg-grafito-50 dark:hover:bg-white/5">
                    <td className="px-4 py-3 font-semibold text-grafito-900 dark:text-white">{r.customers?.full_name ?? '—'}</td>
                    ...
                    <td className="px-4 py-3 text-right">{r.status === 'OPEN' && <button onClick={() => setPay(r)} className="rounded-lg px-2.5 py-1 text-xs font-semibold bg-brand-500 text-white hover:bg-brand-600">Cobrar</button>}</td>
                  </tr>
                )
              })}
            </tbody>
