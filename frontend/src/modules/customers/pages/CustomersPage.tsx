  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button className="flex items-center gap-2 rounded-xl bg-brand-500 px-4 py-2.5 text-sm font-semibold text-grafito-900 dark:text-white hover:bg-brand-600 active:scale-[0.98] transition-all">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {customers.map((c, i) => (
          <div key={i} className="rounded-2xl border border-grafito-200 dark:border-white/5 bg-white dark:bg-grafito-900/60 p-5 backdrop-blur-md space-y-4">
            <div>
              <h3 className="font-bold text-grafito-900 dark:text-white text-lg">{c.name}</h3>
              <p className="text-xs text-brand-400 font-semibold">Cliente Frecuente</p>
            </div>
            <div className="space-y-2 text-sm text-grafito-600 dark:text-grafito-300">
