...
export default function KDSPage() {
  ...
  return (
    <div className="space-y-6 p-6 min-h-screen bg-grafito-950">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white tracking-tight">KDS — Pantalla de Cocina</h1>
            <p className="text-sm text-grafito-400">Control y despacho de comandas de alimentos y bebidas.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {tickets.map((t) => (
          <div key={t.id} className="rounded-2xl border border-white/5 bg-grafito-900 p-5 flex flex-col justify-between space-y-4">
            <div className="space-y-3">
              <div className="flex justify-between items-center border-b border-white/5 pb-2">
                <div>
                  <h3 className="font-bold text-white text-lg">{t.order}</h3>
              ...
              <ul className="space-y-2 text-sm text-grafito-300">
                {t.items.map((item, idx) => (
                  <li key={idx} className="font-medium text-white">{item}</li>
                ))}
              </ul>
            </div>
