    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
        {[
          ...
          <div key={c.name} className="flex items-center justify-between rounded-2xl border border-grafito-200 dark:border-white/5 bg-white dark:bg-grafito-900/60 p-5 backdrop-blur-md">
            <div className="flex items-center gap-3">
              <span className={`h-4 w-4 rounded-full ${c.color}`}></span>
              <div>
                <h3 className="font-bold text-grafito-900 dark:text-white text-sm">{c.name}</h3>
                <p className="text-xs text-grafito-500 dark:text-grafito-400">{c.count} productos</p>
              </div>
            </div>
          </div>
        ))}

        <button className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-grafito-200 dark:border-white/10 bg-white/5 p-5 text-grafito-500 dark:text-grafito-400 hover:border-brand-500/50 hover:bg-brand-500/5 transition-all">
