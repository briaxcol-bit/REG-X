...
import {
  ...
  Store, Star, Puzzle, ChevronLeft, ChevronRight,
  Zap, Bell,
} from 'lucide-react'
...
export function Sidebar({ collapsed, onToggle }: SidebarProps) {
  ...
  return (
    <motion.aside
      ...
      className="fixed left-0 top-0 z-40 flex h-screen flex-col border-r border-white/5 bg-grafito-900 shadow-2xl"
    >
      {/* ── Logo ─────────────────────────────────────────── */}
      <div className="flex h-16 items-center justify-between px-4 border-b border-white/5">
          ...
          {!collapsed && (
            ...
            >
              ...
              <div className="flex flex-col">
                <span className="text-sm font-bold text-white">REG-X</span>
                <span className="text-[10px] text-grafito-400 truncate max-w-[140px]">
        ...
        {!collapsed && (
          <button
            onClick={onToggle}
            className="rounded-md p-1 text-grafito-400 hover:bg-white/5 hover:text-white transition-colors"
          >
      ...
      <nav className="flex-1 overflow-y-auto py-4 scrollbar-thin scrollbar-thumb-white/10">
        {groups.map((group) => (
          <div key={group} className="mb-4">
            {!collapsed && (
              <p className="mb-1 px-4 text-[10px] font-semibold uppercase tracking-widest text-grafito-500">
                {group}
              </p>
            )}
            {visibleItems
                  ...
                  <NavLink
                    ...
                    className={cn(
                      ...
                      isActive
                        ...
                        : 'text-grafito-300 hover:bg-white/5 hover:text-white',
                    )}
                  >
                    {isActive && (
                      <motion.span
      ...
      {collapsed && (
        <button
          onClick={onToggle}
          className="flex items-center justify-center p-4 text-grafito-400 hover:text-white transition-colors border-t border-white/5"
        >
