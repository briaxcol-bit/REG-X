-- ============================================================
-- REG-X — Migration 061: create_sale_transaction v5
-- ------------------------------------------------------------
-- SEGURIDAD: hasta ahora la función confiaba en los totales que
-- enviaba el cliente. Un frontend manipulado podía registrar una
-- venta con total $1. Ahora el servidor RECALCULA los totales a
-- partir de los ítems y rechaza la venta si no cuadran:
--
--   subtotal       = Σ quantity × unit_price
--   tax_total      = Σ tax_amount
--   discount_total = Σ discount_amount
--   total          = max(0, subtotal + tax_total - discount_total)
--
-- (misma fórmula del frontend — CheckoutModal / pos.store).
-- Tolerancia de 0.5 por redondeo de punto flotante.
-- Además: cantidades > 0, precios >= 0, y si la venta nace
-- COMPLETED con pagos, la suma de pagos debe cubrir el total
-- (los pagos pueden exceder: propina va incluida en el pago).
--
-- El resto de la lógica es idéntico a v4 (migración 040).
...
DECLARE
  ...
  v_pay_sum       NUMERIC;
  v_items_ok      BOOLEAN;
  c_tol           CONSTANT NUMERIC := 0.5;   -- tolerancia de redondeo
  -- side-effects
  v_method       TEXT;
  v_amount       NUMERIC;
...
BEGIN
  ...
  -- 1b) VALIDACIÓN DE TOTALES (v5): el servidor no confía en el cliente.
  ...
  SELECT
    ...
    -- sanidad por ítem: cantidades positivas y montos no negativos
    bool_and(
  ...
  -- Si la venta nace COMPLETED con pagos, deben cubrir el total.
  -- (Pueden excederlo: la propina viaja dentro del monto pagado.)
