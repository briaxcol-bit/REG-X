-- ═══════════════════════════════════════════════════════════════════
-- 041: Guardar email en user_profiles para leerlo sin RPC especial
-- Ejecutar en Supabase → SQL Editor
-- ═══════════════════════════════════════════════════════════════════

-- 1. Agregar columna email a user_profiles (idempotente)
...
-- 3. Recrear add_employee_to_tenant para que guarde el email
DROP FUNCTION IF EXISTS add_employee_to_tenant(TEXT,TEXT,TEXT,TEXT,UUID,UUID,TEXT,TEXT,TEXT) CASCADE;
...
BEGIN
  ...
  INSERT INTO user_profiles (id, full_name, email, phone, cedula, custom_role)
  VALUES (v_user_id, p_full_name, lower(trim(p_email)), p_phone, p_cedula, p_custom_role)
...
-- 4. Recrear update_employee_profile para que también actualice email en user_profiles
DROP FUNCTION IF EXISTS update_employee_profile(UUID,TEXT,UUID,TEXT,TEXT,TEXT,TEXT,UUID,TEXT,TEXT) CASCADE;
...
BEGIN
  ...
  -- Perfil (incluye email ahora)
  UPDATE user_profiles
    SET full_name   = p_full_name,
        phone       = p_phone,
        cedula      = p_cedula,
        custom_role = p_custom_role,
        email       = CASE WHEN p_email IS NOT NULL AND trim(p_email) <> ''
